...
```
Recent Changes:
- July 26, 2025. ✅ MIGRATION: Successfully migrated project from Replit Agent to standard Replit environment with blocked senders functionality fix
- July 26, 2025. ✅ FIXED: BlockedSendersPanel translation function error - corrected useLanguage hook destructuring to properly access translation function
- July 26, 2025. ✅ RESOLVED: "ReferenceError: t is not defined" error in CONFIGURAÇÕES > BLOQUEADOS section by adding proper translation function import
- July 25, 2025. ✅ REMOVED: "Move to Spam" and "Archive" options from email content 3-dot menu as requested by user - menu now contains only Reply, Star/Unstar, Print, and Block Sender options
- July 25, 2025. ✅ FIXED: Storage production getFolders method - now properly includes all system folders (Inbox, Archive, Sent, Drafts, Junk, Trash) alongside custom folders
- July 25, 2025. ✅ ENHANCED: Self-blocking prevention - corrected validation to compare email.fromAddress with currentUser.email instead of username
- July 25, 2025. ✅ IMPROVED: Cache invalidation system - added await and refetchQueries for immediate UI updates after email operations without requiring manual refresh
- July 25, 2025. ✅ MIGRATION: Successfully migrated project from Replit Agent to standard Replit environment with email actions fixes
- July 25, 2025. ✅ FIXED: Cache invalidation after blocking sender - UI now updates immediately in all folders after blocking email addresses
- July 25, 2025. ✅ SECURITY: Added validation to prevent users from blocking their own email address with proper error message
- July 25, 2025. ✅ FIXED: "Move to Spam" and "Archive" buttons in email content 3-dot menu now working properly with function calls
- July 25, 2025. ✅ FIXED: Critical language configuration bug - corrected useLanguage hook parameter passing to resolve "language is not defined" error when opening emails
- July 25, 2025. ✅ ENHANCED: Language system now working properly with Portuguese (pt) as default language and proper translation function calls
- July 25, 2025. ✅ FIXED: Block Sender functionality error - added missing createBlockedSender function to production storage with proper MySQL implementation
- July 25, 2025. ✅ ENHANCED: Reply functionality now properly prefills compose modal with original email data for replies and forwards
- July 25, 2025. ✅ IMPROVED: Folder-based action restrictions - hide inappropriate buttons (Reply/Spam/Archive) in DRAFTS and SENT folders
- July 25, 2025. ✅ RESOLVED: All TypeScript boolean to number conversion errors for MySQL tinyint compatibility
- July 24, 2025. ✅ SECURITY: Fixed critical search vulnerability - added explicit userId filtering in storage layer to prevent cross-user email access
- July 24, 2025. ✅ SECURITY: Enhanced search endpoint with double userId authentication verification (routes + storage)
- July 24, 2025. ✅ SEARCH: Confirmed search behavior consistency - all folder types (system and custom) require ENTER key activation
- July 24, 2025. ✅ SEARCH: Verified no automatic real-time search - only explicit search execution via ENTER or button click
- July 24, 2025. ✅ PAGINATION: Confirmed custom folders support full pagination, filtering, and search functionality
- July 24, 2025. ✅ LOGGING: Added comprehensive security logging to storage layer for monitoring user access patterns
- July 24, 2025. ✅ DATETIME: Implemented complete date/time configuration system with Brazilian format (dd/MM/yyyy) and São Paulo timezone - includes server-side configuration, MySQL timezone sync, and frontend utilities  
- July 24, 2025. ✅ ATTACHMENT ICON: Implemented attachment icon display in email list using attachments field logic instead of separate hasAttachments property
- July 24, 2025. ✅ MIGRATED: Successfully migrated project from Replit Agent to standard Replit environment with enhanced search functionality and improved encryption
- July 24, 2025. ✅ SECURITY: Implemented fromAddress field encryption for better data protection - all email addresses now properly encrypted in database
- July 24, 2025. ✅ ENHANCED: Improved search functionality with partial matching support - searches like "Lucas" now find "Lucas da Silva" across all encrypted fields
- July 24, 2025. ✅ IMPROVED: Search now works properly with encrypted fields (subject, body, fromAddress, toAddress) while maintaining security
- July 24, 2025. ✅ COMPLETED: Updated both development and production storage systems with consistent search and encryption capabilities
- July 22, 2025. ✅ COMPLETED: SMTP Email Sending System with Dynamic User Authentication - implemented complete email delivery functionality with nodemailer integration using each user's individual email credentials from database
- July 22, 2025. ✅ ENHANCED: Email system uses user-specific SMTP authentication - each user's email and encrypted password from database used for SMTP auth, showing real sender addresses instead of fixed account
- July 22, 2025. ✅ ADDED: Dynamic SMTP connection testing with user credentials at /api/smtp/test and /api/smtp/test-send endpoints for testing email delivery with individual user authentication
- July 22, 2025. ✅ MIGRATED: Successfully migrated from Replit Agent to standard Replit environment with full SMTP email sending functionality
- July 21, 2025. ✅ COMPLETED: File size error modal system - completely replaced alert() calls with proper React modal component for file size limit warnings (5MB), added Portuguese/English translations, modal uses z-index 150000 to display above compose modal (z-50 to z-100)
- July 21, 2025. ✅ FIXED: All alert() dialogs in compose modal replaced with proper UI components - file size validation uses FileSizeErrorModal, server errors use toast notifications with destructive variant
- July 21, 2025. ✅ IMPROVED: File attachment validation now shows user-friendly modal with proper translations instead of browser alert, includes file name in error message
- July 21, 2025. ✅ IMPROVED: File attachment validation now shows user-friendly modal with proper translations instead of browser alert, includes file name in error message
- July 21, 2025. ✅ COMPLETED: Successfully migrated project from Replit Agent to standard Replit environment with comprehensive functionality improvements
- July 21, 2025. ✅ FIXED: Delete confirmation modal system for special folders (DRAFTS, SENT, TRASH, SPAM) - resolved case-sensitivity bug and circular reference issue, implemented bulk deletion support for multiple email selection
- July 21, 2025. ✅ ENHANCED: Individual and batch permanent deletion now works correctly with proper confirmation modals and automatic UI refresh
- July 21, 2025. ✅ IMPROVED: Bulk move folder options now properly filtered and translated - shows only valid target folders (excludes current folder, sent/drafts), with proper Portuguese translations (Caixa de Entrada, Arquivo, Spam, Lixeira)
- July 21, 2025. ✅ UI FIX: Fixed compose modal z-index layering issue - maximized compose box now properly displays above top-bar with conditional z-index (z-100 when expanded, z-50 normal)
