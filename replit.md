...
```
Recent Changes:
- July 30, 2025. ✅ CRITICAL FIX: Fixed inline image display bug in sent emails - images pasted with CTRL+V now properly appear in received emails instead of showing empty <img> tags
- July 30, 2025. ✅ ENHANCED: Improved ReactQuill image insertion to preserve data-inline-id attributes for proper CID reference replacement
- July 30, 2025. ✅ IMPROVED: Added automatic extraction of inline images from HTML body when not explicitly provided, ensuring all data URLs are processed
- July 30, 2025. ✅ MIGRATION: Successfully migrated project from Replit Agent to standard Replit environment with comprehensive inline image fixes
- July 29, 2025. ✅ IMPLEMENTED: Complete inline image pasting functionality with CTRL+V in compose modal - images are automatically detected, converted to base64, inserted visually in ReactQuill editor, and processed by SMTP service with CID references for proper email delivery
- July 29, 2025. ✅ ENHANCED: SMTP service now handles inline images by converting data URLs to Content-ID (CID) references, allowing images to be properly embedded in sent emails instead of appearing as attachments
- July 29, 2025. ✅ UPDATED: ReactQuill configuration to support image format and added proper ref handling for programmatic image insertion using Quill's insertEmbed API
- July 26, 2025. ✅ REMOVED: "Mover para Pasta" (Move to Folder) option from email content action buttons as requested by user - email actions now show only Mark as Read/Unread, Delete, Manage Tags, and More Actions
