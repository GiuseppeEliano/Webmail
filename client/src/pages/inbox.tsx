...
export default function Inbox() {
  ...
  // Use search results only when search was explicitly executed, otherwise use paginated emails
              ...
              >
                <EmailList
                  ...
                  onArchive={async (email) => {
                    try {
                      // Move to trash folder (assuming folder id 5 is trash)
                      const trashFolder = folders.find(
                        (f) => f.systemType === "trash",
                      );
                      if (trashFolder) {
                        await moveEmail({
                          emailId: email.id,
            ...
            <div className="email-content-container flex-1 overflow-hidden">
              {selectedEmail ? (
                <EmailContent
                  ...
                  onArchive={async (email) => {
                    try {
                      const trashFolder = folders.find(f => f.systemType === "trash");
                      if (trashFolder) {
                        await moveEmail({ emailId: email.id, folderId: trashFolder.id });
                        if (selectedEmail?.id === email.id) {
                          setSelectedEmail(null);
                        }
                      }
        ...
        editingDraft={
          composeContext.type === "compose" && composeContext.email?.isDraft
            ? {
                ...
                attachments: composeContext.email.attachments || [],
              }
            : undefined
        }
        replyTo={
