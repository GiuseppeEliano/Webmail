...
export class MemStorage implements IStorage {
  ...
  private initializeDefaultData() {
    ...
    const systemFolders: Folder[] = [
      ...
      { id: 2, userId: 1, name: "Starred", type: "system", systemType: "starred", color: "#f59e0b", icon: "star", createdAt: new Date() },
      { id: 3, userId: 1, name: "Sent", type: "system", systemType: "sent", color: "#10b981", icon: "send", createdAt: new Date() },
      { id: 4, userId: 1, name: "Drafts", type: "system", systemType: "drafts", color: "#8b5cf6", icon: "file-text", createdAt: new Date() },
      { id: 5, userId: 1, name: "Junk", type: "system", systemType: "junk", color: "#f97316", icon: "alert-triangle", createdAt: new Date() },
      { id: 6, userId: 1, name: "Trash", type: "system", systemType: "trash", color: "#ef4444", icon: "trash2", createdAt: new Date() },
      { id: 7, userId: 1, name: "Work", type: "custom", systemType: null, color: "#3b82f6", icon: "briefcase", createdAt: new Date() },
      { id: 8, userId: 1, name: "Personal", type: "custom", systemType: null, color: "#ec4899", icon: "heart", createdAt: new Date() },
    ];

    systemFolders.forEach(folder => this.folders.set(folder.id, folder));
    this.currentFolderId = 9;
  ...
  async deleteEmail(id: number): Promise<boolean> {
    ...
    // If email is already in trash, delete permanently
    if (email.folderId === trashFolder.id) {
      return this.emails.delete(id);
    }
    
    // If email is a draft, delete permanently (don't move to trash)
    if (draftsFolder && email.folderId === draftsFolder.id) {
      console.log(`Permanently deleting draft email with ID: ${id}`);
      return this.emails.delete(id);
    }
  ...
  async searchEmails(userId: number, query: string): Promise<Email[]> {
    ...
    // Get all user emails and decrypt them for search
    const userEmails = Array.from(this.emails.values())
      .filter(email => email.userId === userId)
      .map(email => this.decryptEmailForUser(email, userId));
    
    // Filter based on decrypted content
