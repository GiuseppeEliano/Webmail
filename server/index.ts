...
if (process.env.DB_HOST) {
  ...
  (async () => {
    try {
      ...
      `);
      console.log('✅ Sessions table verified/created with timezone configuration');
    } catch (error) {
      console.log('⚠️ Error creating sessions table:', error instanceof Error ? error.message : 'Unknown error');
    }
  })();
